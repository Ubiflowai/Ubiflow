...
export default function FloorPlanDesigner({ currentRoomType, onBedCountChange }: DesignerProps) {
  ...
  const exportToPDF = () => {
      ...
      // Header
      pdf.setFontSize(22);
      pdf.setTextColor("#282828"); // Fixed: String color
      pdf.text("UbiFlow Multi-Gas Report", 15, y);
      y += 10;
      pdf.setFontSize(10);
      pdf.setTextColor("#646464"); // Fixed: String color
      ...
      pdf.text("Bill of Materials (Pipe Schedule)", 15, y);
      y += 8;
      ...
      ['O2', 'MA4', 'VAC'].forEach((layerKey) => {
          ...
          // --- FIX: USE HEX STRING INSTEAD OF NUMBER ---
          pdf.setFillColor("#F0F0F0"); // Light Gray Hex Code
    ...
    <div className="w-full p-6 bg-white rounded-xl border-2 border-slate-200 mt-8">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
            <button onClick={exportToPDF} className="bg-red-600 text-white px-3 py-2 rounded text-sm font-bold hover:bg-red-700 shadow-sm">📄 Multi-Gas PDF</button>
        ...
        {editMode === 'PIPE' ? (
            <>
                ...
                <div className="flex items-center bg-white border border-slate-300 rounded px-2 py-1 mr-2">
                    <span className="text-xs font-bold text-slate-500 mr-2">ACTIVE LAYER:</span>
                    <select 
                        value={activeLayer} 
                        onChange={(e) => setActiveLayer(e.target.value as any)} 
                        className="text-sm font-bold outline-none cursor-pointer"
                        style={{ color: GAS_LAYERS[activeLayer].color }}
                    >
                ...
                {/* DYNAMIC PIPE BUTTON COLOR */}
                <button 
                    onClick={() => { setDrawPipeMode(!drawPipeMode); setSelectedStartId(null); }} 
                    className={`px-3 py-2 rounded font-bold text-sm border-2 transition-colors`}
                    style={{
                        backgroundColor: drawPipeMode ? GAS_LAYERS[activeLayer].color : 'white',
                        color: drawPipeMode ? 'white' : GAS_LAYERS[activeLayer].color,
                        borderColor: GAS_LAYERS[activeLayer].color
                    }}
                >
                    {drawPipeMode ? `🔴 Connecting ${activeLayer}...` : `Draw ${activeLayer} Pipe`}
                </button>
            ...
            <>
                ...
                <div className="flex gap-1 bg-white rounded-md border p-1">
                    <button onClick={() => setCadTool('NONE')} className={`p-2 rounded ${cadTool === 'NONE' ? 'bg-slate-200' : 'hover:bg-slate-100'}`} title="Select">👆</button>
                    <button onClick={() => setCadTool('LINE')} className={`p-2 rounded ${cadTool === 'LINE' ? 'bg-slate-200' : 'hover:bg-slate-100'}`} title="Line">📏</button>
                    <button onClick={() => setCadTool('RECT')} className={`p-2 rounded ${cadTool === 'RECT' ? 'bg-slate-200' : 'hover:bg-slate-100'}`} title="Rect">⬜</button>
                    <button onClick={() => setCadTool('TEXT')} className={`p-2 rounded ${cadTool === 'TEXT' ? 'bg-slate-200' : 'hover:bg-slate-100'}`} title="Text">A̱</button>
                </div>
                ...
                return (
                    <Group key={conn.id} opacity={opacity}>
                        ...
                        {isSelected && editMode === 'PIPE' && (
                            <Circle x={midX} y={(start.y+end.y)/2} radius={6} fill="#ef4444" stroke="white" strokeWidth={2} draggable onDragMove={(e) => { setConnections(connections.map(c => c.id === conn.id ? { ...c, bendOffset: e.target.x() - (start.x + end.x) / 2 } : c)); }} />
                        )}
                    </Group>
                );
            })}
      ...
      <div className={`mt-3 p-2 text-center text-xs font-mono rounded-b-lg border-t-2 ${editMode === 'CAD' ? 'bg-orange-50 text-orange-800 border-orange-200' : 'bg-blue-50 text-blue-800 border-blue-200'}`}>
          {editMode === 'CAD' ? (cadTool === 'NONE' ? "CAD: Select/Drag items." : `DRAWING ${cadTool}...`) : "PIPE: Select Active Layer above. Click 'Draw Pipe' to connect."}
      </div>
    </div>
  );
}